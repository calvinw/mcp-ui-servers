"""
Company To Company UI FastAPI App - Separate from MCP server
...
ui_app = FastAPI(title="Company To Company UI")

# Create WebSocket manager
manager = ConnectionManager()

# Import company server functions for WebSocket handling
from company_to_company_server import set_websocket_manager, handle_state_response
...
# UI route
@ui_app.get("/")
async def serve_company_interface(request: Request):
    """Serve the Company To Company interface with session ID"""
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    logger.info(f"Company To Company WebSocket connection attempt with session_id='{session_id}'")
    ...
    try:
        ...
        logger.error(f"Company To Company WebSocket error: {e}")
        manager.disconnect(websocket)
