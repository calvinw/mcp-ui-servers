"""
Company To Company SSE Server - Now using shared ui_app.py
...
import uvicorn
import logging
from company_to_company_server import mcp, set_websocket_manager
...
app = FastAPI(title="Company To Company MCP Server with UI")
...
# Mount the UI app at /company_to_company path FIRST (specific routes before catch-all)
app.mount("/company_to_company", ui_app)
...
if __name__ == "__main__":
    ...
    print(f"""
🏢 Company To Company MCP Server Starting on port {port}!

🌐 Web Interface: http://localhost:{port}/company_to_company
🤖 MCP Endpoint: http://localhost:{port}/sse
⚡ WebSocket: http://localhost:{port}/company_to_company/ws
